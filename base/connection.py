...
class Headers:
    ...
    def keys(self):
        return [header[0].decode() for header in self.__scope["headers"]]

    def as_dict(self) -> dict:
        return {h[0].decode(): h[1].decode() for h in self.__scope["headers"]}
...
class QueryParams:
    def __init__(self, query_string: str):
        self.__dict = dict(parse.parse_qsl(query_string))

    def keys(self):
        return self.__dict.keys()

    def get(self, item, default=None):
        return self.__dict.get(item, default)
...
class WebSocket:
    def __init__(self, scope, receive, send):
        ...
        self.__send = send
        self.__client_state = State.CONNECTING
        self.__app_state = State.CONNECTING

    @property
    def headers(self):
        return Headers(self.__scope)
    ...
    def query_params(self):
        return QueryParams(self.__scope["query_string"].decode())
    ...
    async def accept(self, subprotocol: str = None):
        ...
        """
        if self.__client_state == State.CONNECTING:
            await self.receive()
    ...
    async def send(self, message: t.Mapping):
        if self.__app_state == State.DISCONNECTED:
            raise RuntimeError("WebSocket is disconnected.")

        if self.__app_state == State.CONNECTING:
            ...
            if message["type"] == SendEvent.CLOSE:
                self.__app_state = State.DISCONNECTED
            else:
                self.__app_state = State.CONNECTED

        elif self.__app_state == State.CONNECTED:
    ...
    async def receive(self):
        if self.__client_state == State.DISCONNECTED:
            raise RuntimeError("WebSocket is disconnected.")
        ...
        if self.__client_state == State.CONNECTING:
            ...
            self.__client_state = State.CONNECTED

        elif self.__client_state == State.CONNECTED:
            ...
            if message["type"] == ReceiveEvent.DISCONNECT:
                self.__client_state = State.DISCONNECTED
